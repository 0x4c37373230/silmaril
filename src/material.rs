...
use crate::{random, Color, HitRecord, Point3, Ray, Vec3};
use std::rc::Rc;
...
impl DiffuseLight {
    pub fn from(color: Color) -> DiffuseLight {
        DiffuseLight {
            emit: Rc::new(SolidColor::new(Some(color))),
        }
    }

    pub fn new(emit: Rc<dyn Texture>) -> DiffuseLight {
        DiffuseLight { emit }
    }
}
